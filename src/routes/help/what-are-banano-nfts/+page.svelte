...
<h2 id="contracts-vs-meta" class="text-2xl mb-4">Smart Contracts vs. Meta Protocols</h2>
...
<p>Well, the Banano NFT protocol uses a <b>meta protocol</b>, <ExternalLink url="https://github.com/Airtune/73-meta-tokens" content="created by the wonderful Airtune"/>. A meta protocol is a set of rules we apply to data that is exchanged according to the rules of a different protocol. <b>To simply explain a meta protocol runs on top of a different protocol</b> (kind of like how the HTTP protocol runs on top of the TCP protocol), <b>and so then benefits from all the strengths of the underlying protocol</b>. We'll discuss this more in the next section.</p>

<img src="https://raw.githubusercontent.com/Airtune/73-meta-tokens/main/diagrams/supply-mint-send-receive-simple.png" alt="A diagram showing the meta protocol"/>

<p><i>The meta protocol above. Don't worry, we'll explain it in <a class="link" href="#protocol-explained">the next next section</a>. Diagram above made by Airtune.</i></p>

<h2 id="meta-benefits-downsides" class="text-2xl mb-4">Meta Protocols Benefits and Downsides</h2>

<p>In the last section, we noted that meta protocols inherit all the strengths of it's underlying protocol. In our case, running a meta protocol on Banano means we get to automatically enjoy <b>fast and feeless NFT transfers</b>! In addition, we get a way to not only store data, but prove that the data came from a specific person (by making blocks on the Banano network).</p>
...
<h2 id="protocol-explained" class="text-2xl mb-4">The Protocol Somewhat Hastily Explained</h2>
