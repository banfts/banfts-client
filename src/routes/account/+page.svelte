  ...
  <div class="flex justify-between w-full items-center space-x-2 py-2 px-3 lg:py-3 lg:px-8">
    ...
    <div class="flex-none avatar">
      <div class="w-8 my-auto bg-neutral rounded">
        <img src="https://monkey.banano.cc/api/v1/monkey/{data.account.address}" alt="{data.account.address}"/>
      </div>
    </div>
  ...
  <div class="pt-4 lg:pt-8">
    <div class="mx-auto max-w-4xl px-4 sm:px-4 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8">
      <div class="aspect-h-1 aspect-w-1 lg:aspect-h-1 lg:aspect-w-1 overflow-hidden rounded">
        <img src="{data}" alt="Model wearing plain white basic tee." class="h-full w-full object-cover object-center">
      </div>

      <div class="mt-4 lg:row-span-1 lg:mt-0">
        <h2 class="sr-only">Account information</h2>
        <p class="text-3xl tracking-tight text-gray-300 mb-4">Account</p>

        <div>
          <h3 class="sr-only">Description</h3>
          <div class="space-y-6">
            <p class="text-base text-gray-400">{data.account?.network?.balance_decimal}</p>
          </div>
        </div>
        ...
        <div class="mt-8">
          ...
          <p class="text-gray-300 truncate">{data.account.trades.length}</p>
          <span class="text-gray-500 text-xs">Total Listings</span>
          <p class="text-gray-300 truncate">{data.account.listings.length}</p>
          <span class="text-gray-500 text-xs">Total Assets</span>
          <p class="text-gray-300 truncate">{data.account.assets.length}</p>
        </div>
      </div>
