<script>
  ...
  async function getMinted() {
    minted = (await (await fetch(`${API_URL}/assets/supply/${data.info.supply_hash}/assets?page_num=${pageNum}&per_page=8`)).json()).assets.minted;
  }
  ...
  onMount(() => {
    if (navigator?.clipboard) {
      if (document.getElementById("copy-supply-hash")) {
        document.getElementById("copy-supply-hash").onclick = () => {
          navigator.clipboard.writeText(data.info.supply_hash);
        };
      }
    }
  });
...
<svelte:head>
  <title>{data.info ? `Supply ${data.info.supply_hash}` : "Error"}</title>
</svelte:head>
  ...
  <div class="flex flex-col min-h-full bg-base-300 rounded-box shadow mt-4">
    <div class="flex justify-between w-full items-center space-x-2 py-2 px-3 lg:py-3 lg:px-8">
      <div class="flex-1 max-w-full overflow-hidden">
        <h1 class="me-auto text-lg font-light tracking-tight text-gray-400 sm:text-xl truncate">SUPPLY / {data.info.supply_hash}</h1>
      </div>
      <div class="flex-none self-center">
        <ul>
          <li class="tooltip tooltip-bottom" data-tip="Copy to clipboard">
            <button id="copy-supply-hash" class="btn btn-ghost btn-sm btn-circle" href="#">
    ...
    <div class="pt-4 lg:pt-8">
      <div class="mx-auto max-w-4xl px-4 sm:px-4 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8 lg:px-8">
        ...
        <div class="mt-4 lg:row-span-1 lg:mt-0">
          ...
          <p class="text-3xl tracking-tight text-gray-400 dark:text-gray-300 mb-4">{data.info.asset_supply.nft_metadata.name}</p>

          <div>
            <h3 class="sr-only">Description</h3>
            <div class="space-y-6">
              <p class="text-base text-gray-400">{data.info.asset_supply.nft_metadata.description}</p>
            </div>
          </div>
      ...
      {#each minted as mint}
        <div class="group">
          <div class="card card-compact w-100 bg-base-100 shadow-xl">
            <div class="card-body">
              <h2 class="card-title"><a class="link" href="/explorer/assets?mint_hash={mint.mint_hash}">{data.info.asset_supply.nft_metadata.name}</a></h2>
              <div class="flex">
