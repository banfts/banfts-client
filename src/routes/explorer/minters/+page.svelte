...
<svelte:head>
  <title>{data.found ? (data.address ? `Minter ${data.address}` : "All Minters") : "Error"}</title>
</svelte:head>
...
{#if data.found}
  ...
  {:else}
    ...
    <div class="my-4 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8">
      {#each data.allMinters as minter}
        <div class="group">
          <div class="card card-compact w-100 bg-base-100 shadow-xl p-2">
            <h3 class="text-xl"><a class="link" href="/explorer/minters?address={minter.address}">{ minter.name ? minter.name : "No Name" }</a></h3>
            <div>
              <span><a class="link" href="/explorer/addresses?address={minter.address}">{minter.address.replace(minter.address.slice(10, -4), "...")}</a></span>
              <p>{ minter.description ? minter.description : "No Description" }</p>
            </div>
          </div>
        </div>
      {/each}
