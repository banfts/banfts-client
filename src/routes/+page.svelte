<div class="hero min-h-full bg-base-300 rounded-box shadow my-4">
  <div class="hero-content text-center">
    <div class="max-w-md">
      <h1 class="text-5xl font-bold">Banfts Marketplace</h1>
      <p class="py-6">Buy and sell Banano NFTs securely and efforlessly! No registration required.</p>
      <a class="btn btn-primary btn-sm" href="/auth/representative">Auth with representative</a>
      <a class="btn btn-primary btn-sm" href="/auth/signature">Auth with signature</a>
    </div>
  </div>
</div>

<style lang="postcss">
  /* :global(html) {
    background-color: theme("colors.gray.100");
  } */
</style>